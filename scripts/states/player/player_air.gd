...
func enter():
	print("PlayerAir state entered")
	player.get_node("AnimationPlayer").play("jump")

func exit():
	# Optionally handle any cleanup when exiting the air state
	pass
...
func update(delta: float):
	...
	if Input.is_action_just_pressed("melee_attack"):
		Transitioned.emit(self, "attacking")  # Transition to attacking state if melee attack is pressed during air
